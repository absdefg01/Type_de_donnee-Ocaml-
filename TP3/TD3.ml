...
let rec lookup_assoc_tr cle lst= 
	match lst with
	|[] -> None
	| (cle', valeur) :: r -> 
	if cle = cle' 
	...
	then None
	else lookup_assoc_tr cle r
;;
(*Explication : cette fonction permet de retourner la valeur correspondante à la clé dans la liste*)

(*Pour tester : cette fonction contient deux paramètres : cle et liste, donc on doit entrer la clé pour chercher la valeur correspondante dans la liste et la liste sous forme (cle,valeur)::r*)
let test = (1,11)::(2,12)::(3,13)::[];;
lookup_assoc_tr 2 test;;

let rec add_assoc_tr cle valeur lst = 
  match lst with
    |[]->[(cle,valeur)]
    |(cle',valeur')::r->
      if cle<cle' then 
	(cle,valeur)::lst
      else (cle',valeur')::add_assoc_tr cle valeur r;;
(*Explication : cette fonction permet d'ajouter un élément dans une liste trie*)

(*Pour tester : cette fonction contient trois paramètres : cle, valeur et liste, donc on doit entrer la clé et la valeur qu'on veut ajouter et la liste à ajotuer*)
let test = (1,11)::(2,12)::(3,13)::[];;
add_assoc_tr 4 12 test;;
add_assoc_tr 3 14 test;;

let rec remove_assoc_tr cle lst = 
  match lst with
    |[]->[]
    |(cle',valeur)::r->
      if cle<=cle' then r
      else (cle',valeur)::remove_assoc_tr cle r
;;
(*Explication : cette fonction permet d'enlever le premier élément qui correspond à la clé*)

(*Pour tester : cette fonction contient deux paramètres : cle pour chercher le premier élément à supprimer et la liste*)
let test = (1,11)::(2,12)::(3,13)::[];;
remove_assoc_tr 2 test;;
let test2 = (1,11)::(2,12)::(2,14)::(3,13)::[];;
remove_assoc_tr 2 test2;;

let rec remove_assoc_trV2 cle lst = 
  match lst with
    |[]->[]
    |(cle',valeur)::r->
      if cle=cle' then 
	remove_assoc_trV2 cle r
      else 
	if cle<cle' then 
	  lst
	else 
	  (cle',valeur)::remove_assoc_trV2 cle r
;;
(*Explication : cette fonction permet d'enlever tous les éléments qui correspondent à la clé*)

(*Pour tester : cette fonction contient deux paramètres : cle et liste, donc on doit entrer la clé pour chercher tous les éléments correspondants et la liste à chercher*)
let test = (1,11)::(2,12)::(2,14)::(3,13)::[];;
remove_assoc_trV2 2 test;;
